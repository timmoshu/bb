...
function canMutateQueuedMessagesForThread(
  deps: Pick<QueueDispatchDeps, "db">,
  threadId: string,
): boolean {
  try {
    assertCoordinationContextApplied(deps.db, threadId);
    return true;
  } catch (error) {
    if (
      error instanceof ApiError &&
      error.body.code === "context_not_applied"
    ) {
      return false;
    }
    throw error;
...
function prepareQueuedMessageDispatchWake(
  ...
  switch (wake.kind) {
    ...
    case "provisioning-ended":
      if (!canMutateQueuedMessagesForThread(deps, wake.threadId)) return [];
    ...
    case "host-connected": {
      ...
      for (const threadId of listThreadIdsWithHostOfflineQueueWaits(
        ...
        if (!canMutateQueuedMessagesForThread(deps, threadId)) continue;
...
async function runTurnStartedDispatch(
  ...
  if (!canMutateQueuedMessagesForThread(deps, threadId)) return;
...
async function attemptAutomaticQueuedMessage(
  ...
  if (!canMutateQueuedMessagesForThread(deps, row.threadId)) return;
...
async function runPluginUnregisteredDispatch(
  ...
  for (const row of listQueuedThreadMessagesByWaitHolder(deps.db, holder)) {
    if (!canMutateQueuedMessagesForThread(deps, row.threadId)) continue;
    deps.logger.info(
