...
export async function createThreadFromRequest(
  ...
  if (options.markWorkTogetherCoordination === true) {
    if (request.input.length !== 0 || options.threadId === undefined) {
      throw new Error("Invalid Work Together coordination thread creation");
    }
    const thread = createThreadRecord(deps, {
      environmentId,
      initialStatus: "idle",
      markWorkTogetherCoordination: true,
      request,
