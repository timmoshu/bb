...
describe("work-together thread context apply", () => {
  it("is absent without a token, rejects short tokens, and requires bearer plus a strict body", async () => {
    await withTestHarness(async (harness) => {
      const missingRoute = await harness.app.request(CONTEXT_PATH("thr_missing1"), {
        method: "PUT",
    ...
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      ...
      const missing = await harness.app.request(CONTEXT_PATH("thr_missing1"), {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(body),
      });
      expect(missing.status).toBe(401);
      const missingJson = (await missing.json()) as { code?: string; message?: string };
  ...
  it("returns 404 for PUT on an unknown thread", async () => {
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      ...
      const response = await harness.app.request(CONTEXT_PATH("thr_doesnotexist1"), {
        method: "PUT",
        headers: {
          authorization: `Bearer ${TOKEN}`,
          "content-type": "application/json",
  ...
  it("accepts apply, GET omits bytes, replay is already_accepted, conflicts do not mutate", async () => {
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      const { host } = seedHostSession(harness.deps, { id: "host-wt-context" });
      const envPath = "/tmp/wt-thread-context";
      ...
      expect(getThread(harness.db, threadId)?.environmentId).toBe(environment.id);
      expect(() => assertCoordinationAcpCwd(envPath, "/other-cwd")).toThrow();
      expect(() => assertCoordinationAcpCwd(envPath, envPath)).not.toThrow();
      expect(() => assertCoordinationAcpCwd(envPath, undefined)).not.toThrow();
      ...
      expect(gotBody).toEqual({ requestId: first.requestId, digest: first.digest });
      ...
      expect(getWorkTogetherThreadContext(harness.db, threadId)).toEqual(stored);

      const otherDigest = envelope(`${payload}-other`);
      const sameIdConflict = await harness.app.request(CONTEXT_PATH(threadId), {
        method: "PUT",
        headers: {
          authorization: `Bearer ${TOKEN}`,
          "content-type": "application/json",
      ...
      expect(
        listStoredThreadPromptHistoryRows(harness.db, { threadId, limit: 20 }).some(
          (row) => JSON.stringify(row).includes(payload),
        ),
      ).toBe(false);
    });
  });
  ...
  it("rejects digest mismatch and oversized bytes without storing a row", async () => {
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      const { host } = seedHostSession(harness.deps, { id: "host-wt-context-bad" });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
      ...
      expect(getWorkTogetherThreadContext(harness.db, threadId)?.digest).toBeNull();
  ...
  it("blocks coordination send before apply and allows send after apply without prepending envelope", async () => {
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      const { host } = seedHostSession(harness.deps, { id: "host-wt-context-send" });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
        path: "/tmp/wt-thread-context-send",
      });
      const environment = seedEnvironment(harness.deps, {
        hostId: host.id,
        projectId: project.id,
      ...
      const threadId = ((await created.json()) as { thread: { id: string } }).thread.id;
      const before = await harness.app.request(`/api/v1/threads/${threadId}/send`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
      ...
      expect(listQueuedThreadCommands(harness, "thread.start", threadId)).toHaveLength(0);
      ...
      const after = await harness.app.request(`/api/v1/threads/${threadId}/send`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  it("does not require apply for ordinary non-coordination send", async () => {
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      const { host } = seedHostSession(harness.deps, { id: "host-wt-context-plain" });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
      ...
      const sent = await harness.app.request(`/api/v1/threads/${thread.id}/send`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
  ...
  it("GET is 404 context_not_applied before apply", async () => {
    await withTestHarness({ workTogetherIntegrationToken: TOKEN }, async (harness) => {
      const { host } = seedHostSession(harness.deps, { id: "host-wt-context-get" });
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
