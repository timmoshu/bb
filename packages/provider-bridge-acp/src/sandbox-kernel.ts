...
export function isInsideAcpSandboxRoot(
  root: string,
  candidate: string,
): boolean {
...
export function canonicalAcpSandboxDirectory(
  candidate: string,
  hostHome: string,
): string {
  try {
...
export function acpSandboxAncestorDirectories(
  target: string,
  ...
  for (const root of maskedRoots.map((value) => resolve(value))) {
    if (!isInsideAcpSandboxRoot(root, parent)) continue;
    let current = parent;
...
export function acpSandboxBaseArgs(maskedRoots: readonly string[]): string[] {
  const args = [
    "--die-with-parent",
    "--new-session",
    "--unshare-pid",
