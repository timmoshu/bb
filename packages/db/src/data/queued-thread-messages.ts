...
export function releaseStaleQueuedMessageClaims(
  ...
  const staleClaimWhere = and(
    ...
    notExists(
      db
        .select({ threadId: workTogetherThreadContexts.threadId })
        .from(workTogetherThreadContexts)
        .where(
          and(
            sql`${workTogetherThreadContexts.threadId} = ${queuedThreadMessages.threadId}`,
            isNull(workTogetherThreadContexts.digest),
          ),
        ),
    ),
